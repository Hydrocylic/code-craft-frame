...
| **Phase 2: 提案** | 给出调整方案、影响范围 | ❌ 不修改 | 调整报告 |
...
2. **不批量创建内容文件** — 不应当在一次操作中创建 10 个 `notes.md` 并自动填充
3. **模板只复制不填写** — 可以提议"这里需要 meta.yaml"，但只在用户确认后才从模板复制，且不自动填写
4. **区分"移动"和"生成"** — 批量移动文件没问题，批量生成笔记内容不可以
5. **报告要可审核** — 调整报告必须包含 before/after 目录对比，使用户能逐项判断
