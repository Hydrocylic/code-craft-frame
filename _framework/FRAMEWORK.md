...
7. **陌生领域分层推进** — 进入陌生领域时不"补课后再上手"、也不"盲目上手"：保守实践先行（不依赖未知组件）、学习实践层插入（按需补片）、前瞻性决策推迟（"执行时再定"附录）。详见下节「陌生领域执行原则」
...
进入陌生领域时，领域知识（无限）、项目上下文（庞大）与执行带宽（有限）三者不匹配。「先补课」没有落点，「直接上手」没有抓手。五条原则（来源: kotocord FunASR 任务实践，2026-08-16）:

1. **以项目为教材，不以教程为教材** — 陌生领域知识在项目里的形态才是要学的形态。从「读自己项目里相关的现有实现」开始（如学多线程先品读仓库里已有的线程代码），知识锚定在熟悉上下文上，吸收率远高于抽象教程
2. **按需补片，不按域补课** — 不学"Qt 多线程"这个域，只学当前步骤需要的那一片（一个机制、一个 API）。质询清单（详见 task-note-system.md §3.3 品读变体）就是把无限领域裁剪成有限问题的剪刀
3. **分层吸收，路标兜底** — 知识分「知道存在 / 会用 / 理解」三层，当前阶段只要"会用"；暂时理解不了的大领域用 T4 路标记录，多项目轮回后自然反哺（框架边用边迭代的同一逻辑）
4. **决策降负** — 陌生领域的决策不需要全知后决，三个减压阀: 保守默认值优先（mock 先行、最小可行层先行）、决策可逆（plan-log 体系 + 动态调整记录保证追溯，错了改回来就是）、决策记录即可不追求一次正确
5. **节奏控制，锚点回归** — 信息淹没时回到锚点：分层计划就是锚，每层一个目标。被淹没时问自己「我现在只做哪一步」
